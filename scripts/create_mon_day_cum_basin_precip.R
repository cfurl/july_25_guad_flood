...
aoi <- list.files ("C:\\texas_mpe\\july_flood_no_git\\gis", pattern = "\\.shp$")
...
#y<-2008
    ...
    eaa_query <- eaa_rain %>%
      filter(year == y) %>%
      group_by(grib_id,month, day, year, hour) %>%
      summarize(
    ...
    yo <- map_math |>
      ...
      select(month, day, year,hour, daily_cum_cubic_m_across_basin) |>
      distinct () |>
  ...
  write_csv(final,paste0("C:\\texas_mpe\\july_flood_no_git\\",a,"_hourly_averages_july_event.csv"))
}
